...
function formatTime(minutes, seconds) {
  const zeroValue = (value) => String(value).padStart(2, 0);
  return `${zeroValue(minutes)}:${zeroValue(seconds)}`;
}
...
function countVowels(str) {
  ...
  return strLower.split('').filter((char) => 'aeiouy'.includes(char)).length;
}
...
function isPalindrome(str) {
  const symbols = str.replace(/[^\w]/g, '').toLowerCase();
  const lastLen = symbols.length - 1;
  const midLen = symbols.length / 2;
  for (let i = 0; i < midLen; i += 1) {
    if (symbols[i] !== symbols[lastLen - i]) {
      return false;
    }
  }
  return true;
...
function findLongestWord(sentence) {
  const longest = (word1, word2) =>
    word1.length > word2.length ? word1 : word2;
  return sentence.split(' ').reduce((prev, curr) => longest(curr, prev), '');
}
...
function invertCase(str) {
  const strLower = (char) => char.toLowerCase() === char;
  const invertChar = (char) =>
    strLower(char) ? char.toUpperCase() : char.toLowerCase();

  return str.split('').reduce((acc, curr) => acc + invertChar(curr), '');
}
...
function extractNameFromTemplate(value) {
  ...
  const firstName = current[1];
  const lastName = current[2].slice(0, -1);
...
function encodeToRot13(str) {
  return str.replace(
    ...
    (str) =>
      'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz'[
        'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm'.indexOf(str)
      ]
  );
}
...
function getCardId(value) {
  const ranksCard = 'A234567891JQK';
  const symbolsCard = '♣♦♥♠';

  return (
    symbolsCard.indexOf(value.slice(-1)) * ranksCard.length +
    ranksCard.indexOf(value.slice(0, 1))
  );
}
